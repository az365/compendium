...
- synonymes:
  - детерминированный факторный анализ (не путать с [факторным анализом](./math.md#factor) в мат-стате)
  - разложение динамики на факторы
- виды факторного анализа/разложения (согласно [studizba](https://studizba.com/lectures/jekonomika-i-finansy/faktornyj-analiz/44820-ponjatie-tipy-i-zadachi-faktornogo-analiza.html))
  - детерминированный vs стохастический
    - детерминированный - когда связь факторов с результативным показателем носит функциональный характер
    - стохастический - когда связь факторов с результативным показателем является неполной, вероятностной (корреляционной)
  - статический vs динамический 
    - статический - анализ отклонений план-факт на заданную дату 
    - динамический - анализ изменения между двумя временнЫми точками временнОго ряда
  - ретроспективный vs перспективный
    - ретроспективный - за прошлые периоды
    - перспективный - при whatif-анализе
- виды детерминированного анализа - по видам модели связи факторов и таргета
  - аддитивная 
  - мультипликативная 
  - смешанная 
  - многофакторная
- lib:
  - [studizba: Факторный анализ - лекции](https://studizba.com/lectures/jekonomika-i-finansy/faktornyj-analiz/)
    - [studizba: Детерминированный факторный анализ](https://studizba.com/lectures/jekonomika-i-finansy/faktornyj-analiz/44821-determinirovannyj-faktornyj-analiz.html)
...
- intro
  ...
  - terms
    - АБТ
      ...
      - def: 
        - проверка гипотез влияния изменений в продукте/бизнесе на метрики путём сравнения показателей между тестовой (где внедрено изменение) и контрольной (где изменения не было) выборками
        - проведение АБ-тестов (см. AB-experiment)
      - motiv:
        - АБТ позволяет выделить (замерить) эффект конкретного внедрения на фоне других факторов/событий, сравнить эффекты разных версий решения, выбрать наилучшее
        - если на метрику в один момент действует несколько факторов, в общем случае мы не можем утверждать, что её рост/падение в конкретный момент времени обусловлено интересующим нас событием/внедрением, если оно не проходило чероез АБ-тест
        - в некоторых случаях методы АБТ/[CI](#ci) позволяют также ретроспективного оценить эффекты ранее произошедших событий/внедрений
    - AB-experiment
      - syn: pilot, пилот, АБ-тест, АБ-эксперимент, контролируемый эксперимент
      - def: выкатка внедрения/изменения (тритмента, интервенции) на некоторое подмножество объектов: либо на случайную выборку аудитории (в случае онлайн-АБТ), либо на некоторую выборку гео-объектов: магазины, склады, ПВЗ, ... (в случае оффлайн-АБТ)
      - motiv: позволяет получить корректную оценку эффекта на интересующие метрики с помощью методов АБТ (при условии корректности дизайна теста)
    - treatment
      - syn: воздействие, интервенция, тест, тритмент, пилотное внедрение, контролируемое изменение
      - def: 
        - внедрение, затронувшее тестовую выборку в рамках АБТ
        - воздействие, которое исследуется
      - details/comment: может представляться как булев атрибут объекта, показывающий, испытал ли он воздействие
      - usage: используется для определения его причинно-следственного эффекта на интересующий исход 
    - тестовая группа 
      - syn: тест, test, treatment, тестовая выборка, пилот, pilot
    ...
    - сплит
      - syn: split, разбиение, деление выборок
      - def: выполненное в конкретном пилоте разбиение объектов на тестовую и контрольную выборки или результат их построения/моделирования
    - сплитование 
      - syn: splitting
      - def: способ построения тестовой и контрольной выборок, т.е. деления объектов на тестовую и контрольную группы 
    - единица сплитования
      - syn: key, example key, unit, splitting unit
      - def: ключ примера (класс объекта), экземпляры которых делятся на тестовую и контрольную выборки в результате сплитования для проведения АБ-теста
    - online-abt
      - syn: онлайн АБ-тест
      - def: АБТ со сплитом по пользователям (клиентам) или интервалам их активности (сессии, запросы, ...)
      - details/comment: происходит, как правило, на сайте, в приложении, коммуникационных/маркетинговых операциях, персонализированных предложениях
      - motiv:
        - наиболее чувствительный метод АБТ
        - позволяет стат-значимо детектить меньшие изменения значений метрик за меньшее время проведения эксперимента (по сравнению с оффлайн-АБТ)
        - рекомендуется для любых внедрений, где технически возможен сплит по клиентам (или интервалам их активности)
    - offline-abt
      - syn: оффлайн АБ-тест, оффлайн-тест, гео-эксперимент
      - def: эксперимент со сплитом по гео-объектам (филиалы, города, регионы)
      - details/comment: 
        - практически любое внедрение/изменение, реализованное в некоторую дату в некотором подмножестве гео-объектов может рассматриваться как оффлайн-АБТ
        - некоторые пилоты могут рассматриваться одновременно и как онлайн-абт, и как оффлайн-абт, если есть сплит по клиентам и ограничение по гео-объектам
      - motiv: оценивать на метриках эффекты гео-локализуемых внедрений или внешних событий по аналогии с онлайн-АБТ, когда классический эксперимент со сплитом по клиентам невозможен или затруднён
    - контролируемый эксперимент
      - syn: пилот (в строгом смысле)
      - def: целенаправленно задизайненный тест (эксперимент) с заданным образом отобранной тестовой и контрольной выборками 
      - details/comment: тритментом (раскаткой воздействия) управляет инициатор эксперимента, можем проконтролировать, ка4кое именно изменение выкачено
    - ретроспективный тест
      - syn: пилот (в широком смысле)
      - def: ...
    - локальное событие 
      - syn: гео-локализуемое событие реального мира
    - сетап теста 
      - def: физические параметры проведения конкретного эксперимента 
      - contents: 
        - состав внедрения (версия продукта, изменения БП)
        - даты (timestamps, моменты) начала/окончания (известные на текущий момент - планируемые или фактические)
        - единица сплитования (кука пользователя, id клиента, номер телефона, город, регион, ...)  # или это часть дизайна теста?
        - характеристики выборок: размер, версия внедрения/изменения/тритмента (или его отстутствие - в контрольной выборке), ...
        - дополнительные условия/ограничения попадания объектов (пользователей, сессий, запросов) в тест (регионы, сегменты аудитории, параметры запросоов, ...)
    - дизайн теста 
      - def: параметры методики, по которой договорились оценивать эксперимент
      - contents: 
        ...
        - ...
    - p-value 
      - def: вероятность того, что наблюдаемая разница в метриках между группой А и группой Б могла быть случайной
      - details/comment:
        - например, если значение `p=0.05`, можно сказать, что существует `5%` вероятности, что результат получен случайно и не соответствует действительности (не воспроизведётся при повторном тесте)
        - для успешного теста необходимо, чтобы p-value была меньше `0.05`
    - power 
      - syn: мощность теста
      - def: уровень доверия к результатам, которые показывает текущий вариант решения (контроль)
      - details/comment: обычно при получении мощности от `80%` результаты теста считаются достоверными 
    - effect size 
      - syn: размер эффекта
    ...
    - вердикт 
      - syn: результат теста
      - def: главный вывод по результатам эксперимента, признаётся ли пилот (изменение/внедрение) успешным
      - details/comment: включает в себя прикладную интерпретацию прокраса приёмочной метрики, оценку корректности проведения и результата эксперимента  
    - прокрас
      - syn: стат-значимость  
      - def: наличие статистически значимого эффекта в АБ-тесте
      - details/comment: достигается при p-value (для разницы между тестом и контролем) меньше порогового/критического
    - приёмочная метрика 
      - syn: ключевая метрика 
      - def: выбранная до начала теста целевая метрика, по которой договорились оценивать эффект эксперимента
    - АБ-методы
      - def: DA-методы проектирования, проведения, обсчёта/анализа экспериментов 
    - методика эксперимента 
      - def: описание необходимого и достаточного набора параметров проведения и обсчёта эксперимента, включая приёмочную метрику, метод оценки и его параметры (но не ограничиваясь ими) 
      - details/comment: аналогично методике других DA-задач, методика эксперимента может быть представлена как сочетание формализованных гипотез, метрик, методов (и их параметров) 
    - реализация АБ-метода/методики 
      - def: конкретное математическое и программное решение (код), реализующее метод (методику) оценки экспериментов
      - details/comment: на вход принимает сетап теста, на выход даёт прокрасы метрик (величину оцененного влияния и p-value)
    - валидация 
      - syn: валидация методики, метода, реализации метода
      - def: процесс проверки корректности реализованной методики путём её обкатки на данных эталонных экспериментов (голден-сет) и сверки результатов с ранее провалидированными 
      - details/comment: 
        - по результатам валидации более предпочтительной признаётся та методика (реализация), которая:
          - красит тесты в правильную сторону (знак изменения соответствует эталонным оценкам)
...
- Intro
  ...
  - `post` [Иван Юрашку (Сбер) @ Habr: Causal Inference: прозрение и практика. Лекция 1. Основные понятия Causal Inference](https://habr.com/ru/companies/sberbank/articles/847382)
  - курсы 
  ...
  - shapley values
    ...
    - виды Shapley Values ([О.Филиппова](https://www.youtube.com/watch?v=rrreuktS0gk))
      ...
      - Asymmetric Shapley values
        ...
        - `paper` [Christopher Frye et al: Asymmetric Shapley values: incorporating causal knowledge into model-agnostic explainability (2021)](https://arxiv.org/pdf/1910.06358.pdf)
        - `video` [Faculty @ YouTube: Tech talk: More explainable AI with Asymmetric Shapley Values (2021)](https://youtube.com/watch?v=7d13f4UaAn0&t=3061s)
        - приложения
