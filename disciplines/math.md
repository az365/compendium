...
- меры рассеяния
  ...
  - стандартаная ошибка = SD
    - `SD = sigma / sqrt(n)`
  - размах вариации
  - IQR = interquantile range = межквантильный размах
...
- результат [инференса](./datan.md#inference) - статистическое суждение
  ...
  - [кластерный анализ](./ds.md#clustering)
...
- определения
  - `wiki` [Регрессия](https://ru.wikipedia.org/wiki/Регрессия_(математика)) - односторонняя стохастическая зависимость, устанавливающая соответствие между случайными переменными
    - математическое выражение, отражающее связь между зависимой переменной у и независимыми переменными х при условии, что это выражение будет иметь статистическую значимость
  - `wiki` [Регрессионный анализ](https://ru.wikipedia.org/wiki/Регрессионный_анализ) - набор статистических методов исследования влияния одной или нескольких независимых переменных `X1,X2,...,Xp` на зависимую переменную `Y`.
...
- виды регрессии 
  ...
  - квантильная регрессия
    - оценивает условную медиану или другие квантили зависимой переменной 
    - более устойчива к выбросам, по сравнению с линейной регрессией среднего 
    - используется в экологии, когда есть только слабая взаимосвязь между переменными
    - `post` [Артем Черемухин @ Habr: Взглянем на квантильную регрессию](https://habr.com/ru/articles/587720/)
  - модели бинарного выбора 
    - пробит-регрессия
    - логит-регрессия
- литература
  - `post` [О.Седухин @ Habr: Теория вероятностей в машинном обучении. Часть 1: модель регрессии (2023)](https://habr.com/ru/companies/ods/articles/713920/)

<a name="factor"></a>
### Факторный анализ
- term:
  - (в статистике) многомерный метод изучения взаимосвязей между переменными
  - (в аналитике) может упоминаться в значении [факторное разложение](./datan.md#factor)
- related terms:
  - фактор — скрытая переменная, объясняющая связь между совокупностью переменных
  - факторная нагрузка — корреляция между исходной переменной и фактором
  - коммунальность – коэффициент объяснённости отдельного показатель общим фактором
  - собственное число (eigenvalues) – насколько важный фактор можно выделить из набора данных
  - факторное пространство – многомерное пространство, где каждая переменная выражена через факторы
- usage:
  - определение взаимосвязей между переменными
  - объективная R-классификация переменных
  - снижение размерности - сокращение числа переменных, описывающих данные
  - выделение скрытых (латентных) переменных
- principle:
  - предполагается, что известные переменные зависят от меньшего количества неизвестных переменных и случайной ошибки
  - в один фактор объединяются сильно коррелирующие между собой переменные, как следствие происходит перераспределение дисперсии между компонентами
  - коррелированность компонент внутри фактора между собой выше, чем их коррелированность с компонентами из других факторов
- limitations:
  - число наблюдений должно быть не менее чем в два раза больше числа переменных
  - выявленные факторы (скрытые переменные) не всегда имеют прикладную (физическую) интерпретацию
- methods:
  <a name="pca"></a>
  - PCA (principal component analysis, метод главных компонент) - поиск главных компонент (линейных комбинаций переменных), которые объясняют наибольшую долю изменчивости данных
  - PLS (partial least squares, метод наименьших квадратов) - для множественной регрессии на большой размерности
  - MLE (maximum likelihood estimation, метод максимального правдоподобия) - оценка параметров модели на соответствие данным
  - PAF (Метод главных осей)
  - методы вращения факторов: ортогональное (Varimax), наклонное (Oblimin, Promax)
  - альфа-факторизация
  - канонический факторный анализ
- lib
  - `post` [Б.Сиротич @ Я.Практикум: Факторный анализ: что это такое и как его проводить](https://practicum.yandex.ru/blog/chto-takoe-faktornyj-analiz-vidy-i-metody/)
  - `post` [А.Михайлов (Otus) @ Habr: Факторный анализ](https://habr.com/ru/companies/otus/articles/741064/)
  - `synopsis` [ВШЭ @ studfile: Факторный анализ](https://studfile.net/preview/9014670/)
...
- граф
  ...
  - характеристики графа
    ...
    - дерево (связный ациклический граф, у которого между любыми двумя вершинами есть ровно одна простая цепь)
...
- классификации задач
  - согласно [Википедии](https://ru.wikipedia.org/wiki/Оптимизация_(математика))
    ...
    - методы: аналитические (множители Лагранджа, условия Каруша-Куна-Такера), численные, графические
  ...
  - согласно [Habr: Обзор методов численной оптимизации](https://habr.com/ru/post/561128)
    ...
    - по трудоёмкости целевой функции
      ...
      - шумные - когда целевая функции вычисляется неточно
    - по гладкости и наличию частных производных: прямые методы, первого, второго порядка (про гессиан)
      - методы, использующие производную
      - безградиентные (derivative-free) методы
...
- Литература
  - `book` [Ian Goodfellow (MIT, Apple) - Deep Learning](https://www.deeplearningbook.org/)
    - 1 Introduction
  ...
  - `book` [MML-book: Marc Peter Deisenroth, A. Aldo Faisal, Cheng Soon Ong - Mathematics for Machine Learning (2020)](https://mml-book.github.io/)
